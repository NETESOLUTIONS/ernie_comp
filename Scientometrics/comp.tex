...
\begin{abstract}
Computer science has experienced dramatic growth and diversification over the last twenty years. Towards a current understanding of the structure of this discipline, we analyze a cohort of the computer science literature using the DBLP database.  For insight on the features of this cohort and the relationship within its components, we construct article level clusters based on either direct citations or co-citations, and reconcile them with major and minor subject categories in the All Science Journal Classification (Scopus). 
Our study finds complementary insights can be gained by direct citation and co-citation, 
and both point to the dramatic increase in computer science publications and scope.
Our analyses show the emergence of new subfields, including ones that interact with external fields, such as the biological sciences, while some subfields
have remained primarily inward looking.
Thus,  the  division of computer science into three major subfields (hardware, software, and theory) observed by Salton and Bergmark 40 years ago is no longer
an appropriate characterization of the computer science field. 
...
%Analyzing clusters generated using co-citation links is more challenging. 
A comparison of the clusters generated using direct citation and co-citation show interesting contrasts. 
In some cases, 
%The data suggest two co-existing patterns. One in which 
the nodes in a co-citation cluster map largely to one or two direct citation clusters, but  in some other cases, the nodes in a co-citation cluster are taken from a large number of the direct citation clusters. 
%The second, which may account for more than 50\% of the nodes in a co-citation cluster, is where the nodes in co-citation cluster are distributed across all 20 direct citation clusters. 
The latter condition is intriguing in its implications of a web of co-citational links gluing the body of scientific literature together.

Overall we observed that in the 40 years since Salton and Bergmark's landmark paper,
 the field of computer science has not only expanded in volume, it has expanded in its interactions with other fields, and has
also resulted in new subfields.
%Significant interactions with the biological sciences in particular were observed, suggesting that computer science has been stimulated by
%biology and also impacted biology. 
...
This preference may mitigate questions  regarding generalizability and the specific choices we made, such as the
 (i) use of the DBLP dataset and the efficiency of matching to Scopus, which may not capture all aspects of computer science and its interactions with other fields, (ii) the basis for clustering, and (iii) mapping the results of this clustering against a classification designed around journals rather than individual articles. 
Future research should examine the growth and changes in the field of computer science to investigate the changes at a more detailed
level.
\clearpage
